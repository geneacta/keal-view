...
nothing, the machine is usually busy, and six runs took a minute.

It earned its place the first time it ran. A call trace added at every runtime
failure site cost the language +2.6 % on a compiler compiling itself and **+11
to +35 % here**, depending on which of the three columns you read — and the
spread is the finding: the more work a call does, the less the frame pays. A
